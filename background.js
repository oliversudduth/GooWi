...
function installSelectionContextMenu() {
  chrome.contextMenus.removeAll(() => {
    ...
    // surfaced in the service-worker console during development reloads.
...
function insertReaderCss(tabId) {
  return new Promise((resolve, reject) => {
...
function injectReaderScript(tabId) {
  return new Promise((resolve, reject) => {
...
async function showSelectionInTab(tabId, selection) {
  ...
  try {
    await insertReaderCss(tabId);
    await injectReaderScript(tabId);
    await tabsSendMessage(tabId, message);
  } catch {
    // Chrome blocks script injection on protected pages such as chrome:// and
    // the Chrome Web Store. Fail quietly there.
  }
}
