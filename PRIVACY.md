...
On Firefox surfaces where the browser provides a selected-text or link **View in GooWi** context-menu action but blocks ordinary page injection, GooWi may render the requested lookup inside its own Firefox extension sidebar instead. This changes only where the reader is displayed. The resulting lookup term is sent directly to Wikipedia under the existing 75-character and privacy rules. No additional data is sent to the developer.
