...
- **View in GooWi** context-menu lookup for selected words and phrases
  ...
  - Chromium's built-in PDF viewer continues to use the existing injected-reader path where supported
...
v0.8.1 keeps Chromium behavior aligned with the Firefox release. Chromium's existing **View in GooWi** path remains unchanged and already worked alongside the built-in PDF viewer in manual testing. The v0.7.20 matcher remains unchanged.
...
- Minimum supported Chrome version: **99**
- The Chrome build uses a Manifest V3 background service worker.
